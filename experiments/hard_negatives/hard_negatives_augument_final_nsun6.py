# In this script we oversample NSUN2 class by 2x
...
folds_augmented = CURRENT_PATH / "folds_augmented_final_hard"
...
for lower_bound in lower_bounds:
    for upper_bound in upper_bounds:
        ...
        path_val = DATASET_DIR / f"test_set.pickle"
        ...
        for key, value in val_data.items():
            ...
            for i in range(len(value)):
                ...
                seq = seq[50:151-50]
                resized_sequences.append(seq)
            resized_data_val[key] = resized_sequences


        n_negatives = len(resized_data["negative"])
        n_negatives_val = len(resized_data_val["negative"])
        tot = n_negatives + n_negatives_val #85% + 15% initial dataset
        more_nsun2 = 2.0
        quota = {1: tot, 2: int(tot*more_nsun2), 3: tot, 4: tot} # default quota for each class
        ...
        #  Round-robin picking
        selected_all = {4: defaultdict(list), 3: defaultdict(list),
                        2: defaultdict(list), 1: defaultdict(list)}
        ...
        for cls in (4, 3, 2, 1):
            ...
            picked = 0                                   # how many for this cls
            while picked < quota[cls]:
                ...
                # --- record directly by bin ---
                selected_all[cls][bin_ptr].append(i)
        ...
        selected_train = []
        selected_val = []
        # split 85% of the selected sequences for training and 15% for validation
        for cls in (4, 3, 2, 1):
            for b, idx_list in selected_all[cls].items():
                n_train = int(0.85 * len(idx_list))
                selected_train.extend(idx_list[:n_train])
                selected_val  .extend(idx_list[n_train:])


        resized_data["negative"].extend(all_seqs[selected_train].tolist())
        resized_data_val["negative"].extend(all_seqs[selected_val].tolist())



        path_train_augmented = path_threshold / f"train_final_augmented_hard_nsun2x{more_nsun2}.pickle"
        with open(path_train_augmented, "wb") as f:
            pickle.dump(resized_data, f)
        path_val_augmented = path_threshold / f"val_final_augmented_hard_nsun2x{more_nsun2}.pickle"
        with open(path_val_augmented, "wb") as f:
            pickle.dump(resized_data_val, f)
